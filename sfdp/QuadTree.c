...
static node_data node_data_new(int dim, real weight, real *coord, int id){
  node_data nd;
  int i;
  nd = (node_data) MALLOC(sizeof(struct node_data_struct));
  nd->node_weight = weight;
  nd->coord = (real*) MALLOC(sizeof(real)*dim);
  nd->id = id;
...
void QuadTree_get_supernodes(QuadTree qt, real bh, real *point, int nodeid, int *nsuper, 
  ...
  *flag = 0;
  *nsupermax = 10;
  if (!*center) *center = (real*) MALLOC(sizeof(real)*(*nsupermax)*dim);
  if (!*supernode_wgts) *supernode_wgts = (real*) MALLOC(sizeof(real)*(*nsupermax));
  if (!*distances) *distances = (real*) MALLOC(sizeof(real)*(*nsupermax));
...
static real *get_or_alloc_force_qt(QuadTree qt, int dim){
  ...
  if (!force){
    qt->data = (real*) MALLOC(sizeof(real)*dim);
    force = (real*) qt->data;
...
QuadTree QuadTree_new_from_point_list(int dim, int n, int max_level, real *coord, real *weight){
  ...
  xmin = (real*) MALLOC(sizeof(real)*dim);
  xmax = (real*) MALLOC(sizeof(real)*dim);
  center = (real*) MALLOC(sizeof(real)*dim);
